...
<div class=”mainFrame”  style="position: relative; width: 480px; height: 1030px; border:1px solid black; display: inline-block;">
	...
	<h1 style="color: rgba(0, 0, 0, 0.5); margin: 40px auto;">비밀번호 변경</h1>
	
	<form action="changePassword" method="post">
		<input type="hidden" value="${helper_email}" name="helper_email">
		<input id="password1" type="password" placeholder="새 비밀번호 입력"  name="helper_password" required
		...
		<input id="password2" type="password" placeholder="새 비밀번호 확인"  name="helper_password2" required
					 style="background:#fffcfc; color: rgba(0, 0, 0, 0.5); padding: 10px; 
					 margin: 10px; font-size: 20px; font-weight: bold; border-radius: 15px; 
					 border: 1px solid rgba(0, 0, 0, 0.5)">
					 
		 
		<input id="saveBtn" type="submit" class="btn2" value="저장하기" style="width: 287px; height:44px; color: white;
					 background: #A9E5FF; text-align: center; font-size: 20px; padding: 25px auto;
	...
	<script>
		...
		$("#password1, #password2").on('input', function () {
			...
			let saveBtn = $('#saveBtn');
			if(pw1 === pw2) {
				saveBtn.css({"background":"#FF578B"});
			} else {
				saveBtn.css({"background":"#A9E5FF"});
			}
		});
	</script>
