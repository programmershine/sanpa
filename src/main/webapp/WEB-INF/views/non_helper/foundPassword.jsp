...
<div class=”mainFrame”  style="position: relative; width: 480px; height: 1030px; border:1px solid black; display: inline-block;">
	...
	<h1 style="color: rgba(0, 0, 0, 0.5); margin: 40px auto;">비밀번호 찾기</h1>
	<span style="color: rgba(0, 0, 0, 0.4);font-size: 16px; font-weight: bold; text-align: center;margin: 10px">
		입력하신 아이디에 등록된 이메일 주소<br><span style="color: rgba(0, 0, 0, 0.6)">${helper_email}</span>(으)로 <br>인증번호를 발송했습니다.
	</span>
	
	<form action="foundPassword" method="post">
		<input id="authNum" type="text" placeholder="인증코드를 입력해 주세요." maxlength="8"  name="emailCode" required
		...
		<%-- 비밀번호 변경을 위해 이메일 값을 컨트롤러로 보냄 --%>
		<input type="hidden" value="${helper_email}" name="helper_email">

		<Br><br>
					 
		 <!-- 폼 안에 있으니 ajax로 처리함 -->
		 <button id="authBtn" style="font-size: 17px; color:rgba(0, 0, 0, 0.6); text-decoration: underline; font-weight: bold ;
		 							 background: white; border: none">인증코드 재발급</button>


		 <Br>
		<input type="button" id="authConfirmBtn" class="btn2" value="인증코드 확인" style="width: 287px; height:44px; color: white;
					 ...
					 margin-top: 60px;">
		<input type="submit" class="btn2" value="비밀번호 변경하기" style="width: 287px; height:44px; color: white;
					 background: #FF578B; text-align: center; font-size: 20px; padding: 25px auto;
					 border-radius: 15px; box-shadow: 2px 4px 4px 0px rgba(0,0,0,0.4); border: none;
					 margin-top: 60px;">
	</form>
...
<script>
	...
	// "인증번호 다시 받기" 버튼 클릭 이벤트 핸들러
	$("#authBtn").click(function(e) {
		e.preventDefault();
		var email = "${helper_email}";
	...
	// 이전 단계에서 받은 이메일 코드 콘솔창에 띄우기
	var authNum = "${authNum}";
	console.log("인증코드: " + authNum);


	var isEmailCodeConfirmed = false;

	// "인증코드 확인"버튼 클릭 이벤트 핸들러
	$("#authConfirmBtn").click(function(e) {

		e.preventDefault();

		var authNum = $("#authNum").val();
		var msg = "인증코드가 확인되었습니다.";
		$.ajax({
			type: "POST",
			url: "/emailauthCheck",
