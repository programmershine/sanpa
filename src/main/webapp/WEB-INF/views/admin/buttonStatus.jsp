...
<main>
    ...
    <div class="contentcontainer">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="width: 650px"></div>  <p class="contentTitle" style="display: inline-block;"> 버튼상황목록</p><div style="width: 350px"></div>
                ...
                <form style="display: inline-block;">
                    <table>
                        <tr>
                            ...
                            <td>
                                <div class="dropdown">
                                    <select name="searchCondition" class="dropbtn dropbtn2" style="width:100%">
                                        <c:forEach var="option" items="${conditionMap2}">
        ...
        <div class="content">
            <table class="category">
                ...
                <c:forEach var="List" items="${buttonStatus}">
                    <tr class="member" onclick="location.href='admin_motherDetail?mother_id=${List.mother_id}'" style="cursor: pointer">
                        <td>${List.mother_id}</td>
                        <td>${List.helper_name}</td>
                        <td>${List.mother_due_date}</td>
                        <td>
                            <c:choose>
                                <c:when test="${List.mother_emergency_alaram == 2}"><span style="font-weight: bold">B</span></c:when>
                                <c:when test="${List.mother_emergency_alaram == 1}"><span style="font-weight: bold">A</span></c:when>
                                <c:when test="${List.mother_emergency_alaram == 0}"><span style="font-weight: bold">&nbsp;-&nbsp;</span></c:when>
                                <c:otherwise>&nbsp;-&nbsp;</c:otherwise>
                            </c:choose>
                        </td>
                        <td>${List.whenToClick}</td>
                        <td>${List.helper_tel.substring(0,3)}-${List.helper_tel.substring(3,7)}-${List.helper_tel.substring(7,11)}</td>
                        <td>${List.num}</td>
                    </tr>
                </c:forEach>
                </tbody>
...
<script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response, function(i, helper) {
                        var helperRow = '<tr class="member" onclick="location.href=\'admin_motherDetail?mother_id=' + helper.mother_id + '\'" style="cursor: pointer;">' +
                            '<td>' + helper.mother_id + '</td>' +
                            '<td>' + helper.helper_name + '</td>' +
                            '<td>' + helper.mother_due_date + '</td>' +
                            '<td>' + getStatusText(helper.mother_emergency_alaram) + '</td>' +
                            '<td>' + helper.whenToClick + '</td>' +
        ...
        $('#searchKeyword').keyup(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // 서버로부터 받은 사용자 목록 데이터를 순회하면서 화면에 표시합니다.
                    $.each(response, function(i, helper) {
                        var helperRow = '<tr class="member" style="cursor: pointer">' +
                            '<td>' + helper.mother_id + '</td>' +
                            '<td>' + helper.helper_name + '</td>' +
                            '<td>' + helper.mother_due_date + '</td>' +
                            '<td>' + getStatusText(helper.mother_emergency_alaram) + '</td>' +
    ...
    $(document).on('click', '.listTable .member', function() {
        var motherId = $(this).find('td:first').text();
        location.href = 'admin_motherDetail?mother_id=' + motherId;
        console.log('Clicked row with mother_id: ' + motherId);
    });
